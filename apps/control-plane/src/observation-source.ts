...
export class HttpObservationSource implements ObservationSource {
  ...
  private async fetchAcpEvidence(
    ...
    const bridgeToken = await usableBridgeToken(
      { productPool: this.options.productPool, runtimeControlBaseUrl: this.options.runtimeControlBaseUrl, fetchInit: this.deadline(), ...(this.options.logger ? { logger: this.options.logger } : {}) },
      workstreamId,
      session,
      pod.incarnation,
    )
