...
test('suspendSession dematerializes the real Session Runtime and reaches suspended', async () => {
  await withTestDatabase(async (pool) => {
    ...
    try {
      ...
      const decommissionedBefore = broker.decommissionCalls.length
      ...
      assert.equal(broker.decommissionCalls.length, decommissionedBefore + 1, 'suspend must decommission the Session\'s OneCLI Agent')
      // ...the AGENT, never the GRANT: the grant is the standing entitlement a resume provisions from.
