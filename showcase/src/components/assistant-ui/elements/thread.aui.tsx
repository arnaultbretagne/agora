...
import { AgoraTool } from "@/agora/toolkit";
import {
  ExecutionBanner,
  NoticeMessage,
  SendsClosedBanner,
...
const ThreadMessage: FC = () => {
  ...
  if (role === "system") return <NoticeMessage />;
  ...
  return (
    <div className="aui-thread-welcome-root mb-6 flex flex-col px-2">
      <p className="aui-thread-welcome-message-inner fade-in slide-in-from-bottom-1 animate-in fill-mode-both text-2xl font-medium tracking-tight duration-200">
        Nouveau workstream — écrivez la première demande.
      </p>
    </div>
  );
};
        ...
        >
          <ComposerPrimitive.Cancel asChild>
            <Button
              ...
              aria-label={isSending ? "Annuler l'envoi" : "Annuler le tour"}
            >
...
/** Agora : un groupe d'outils s'ouvre dès qu'un de ses outils attend une permission,
 * sinon la demande reste cachée dans le groupe replié. */
const ToolGroupAutoOpen: FC<PropsWithChildren<{ group: ThreadGroupPart }>> = ({
  group,
  children,
}) => {
  const counts = (group as { counts?: { requiresAction?: number } }).counts;
  const needsAnswer =
    (counts?.requiresAction ?? 0) > 0 || group.status.type === "requires-action";
  const [open, setOpen] = useState(needsAnswer);
  const [prev, setPrev] = useState(needsAnswer);
  if (needsAnswer !== prev) {
    setPrev(needsAnswer);
...
const AssistantMessage: FC = () => {
  const {
    ToolFallback: ToolFallbackComponent = AgoraTool,
    ToolGroup,
    ReasoningGroup,
          ...
          {({ part, children }) => {
            switch (part.type) {
              ...
              case "group-tool":
                ...
                return <ToolGroupAutoOpen group={part}>{children}</ToolGroupAutoOpen>;
...
export { AssistantMessage, Composer, ThreadComponentsContext, ThreadMessage, UserMessage };
