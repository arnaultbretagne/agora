...
import { ExecutionBadge } from "@/agora/components";
  ...
  return (
    <div data-slot="aui_thread-list-search" className="relative px-0.5 py-1">
      ...
      <Input
        ...
        aria-label="Chercher un workstream"
        placeholder="Chercher un workstream"
  ...
  return useMemo(() => {
    ...
    const filteredIndices = threadIds
      ...
      .filter(
        ({ id }) =>
          !query ||
          (itemsById.get(id)?.title || "Sans titre")
            .toLowerCase()
            .includes(query),
      )
      .map(({ index }) => index);
    ...
    >
      {isRenaming ? (
        ...
        >
          ...
          >
            <ThreadListItemPrimitive.Title fallback="Sans titre" />
          </span>
          <ExecutionBadge />
          {isRunning && <span className="sr-only">Tour en cours</span>}
        </ThreadListItemPrimitive.Trigger>
      )}
    </ThreadListItemPrimitive.Root>
  );
