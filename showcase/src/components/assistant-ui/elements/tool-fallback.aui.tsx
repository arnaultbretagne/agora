...
function ToolFallbackTrigger({
  ...
  const label = isCancelled ? "Outil annulé" : "Outil";
...
const APPROVAL_OPTION_DEFAULT_LABELS: Record<string, string> = {
  "allow-once": "Autoriser",
  "allow-always": "Toujours autoriser",
  "reject-once": "Refuser",
  "reject-always": "Toujours refuser",
};
  ...
  const answerField = acceptsText ? (
    <div className="aui-tool-fallback-approval-answer flex flex-col items-start gap-2">
      <Textarea
        ...
        placeholder={
          question ? "Votre réponse" : "Ajouter une note à la décision"
        }
      />
      {question && (
...
/** Agora : la décision reste visible une fois la permission répondue. */
function ToolFallbackApprovalOutcome({
  approval,
}: {
  approval?: ToolCallMessagePart["approval"];
}) {
  if (!approval) return null;
  const chosen = approval.options?.find((o) => o.id === approval.optionId);
  const label =
    approval.resolution === "cancelled"
      ? "annulée avec le tour"
  ...
  const refused = approval.approved === false || approval.resolution !== undefined;
  return (
    <p
      data-slot="tool-fallback-approval-outcome"
      className={cn(
        "px-4 text-xs",
        refused ? "text-muted-foreground" : "text-foreground",
      )}
    >
      Permission : <b>{label}</b>
...
const ToolFallbackImpl: ToolCallMessagePartComponent = ({
  ...
  return (
    <ToolFallbackRoot open={open} onOpenChange={setOpen}>
      <ToolFallbackTrigger
        toolName={(artifact as { title?: string } | undefined)?.title ?? toolName}
        status={status}
      />
      <ToolFallbackContent>
        ...
        <ToolFallbackApprovalOutcome approval={approval} />
...
const ToolFallback = memo(
  ...
  ApprovalOutcome: typeof ToolFallbackApprovalOutcome;
};

ToolFallback.displayName = "ToolFallback";
ToolFallback.ApprovalOutcome = ToolFallbackApprovalOutcome;
