...
// A structurally-valid JWT shape is enough for these tests — content doesn't matter here, only
// that ensureCodexAuthStub round-trips whatever identity claims the stub carries.
const FAKE_ID_TOKEN = `${Buffer.from(JSON.stringify({ alg: 'none' })).toString('base64url')}.${Buffer.from(JSON.stringify({ sub: 'test-account' })).toString('base64url')}.`

async function seedAuthStub(idToken: string, accountId: string): Promise<string> {
  const stubsDir = await mkdtemp(join(tmpdir(), 'onecli-stubs-'))
  await writeFile(join(stubsDir, 'codex-auth-json'), JSON.stringify({ idToken, accountId }))
  return stubsDir
}
...
test('required: ensureCodexAuthStub builds auth.json from the stub-supplied identity, with fixed non-secret bearer placeholders', async () => {
  const home = await mkdtemp(join(tmpdir(), 'codex-auth-stub-test-'))
  const stubsDir = await seedAuthStub(FAKE_ID_TOKEN, 'account-abc-123')
  await ensureCodexAuthStub(home, stubsDir)
  const raw = await readFile(join(home, '.codex', 'auth.json'), 'utf8')
  const parsed = JSON.parse(raw) as { tokens: { id_token: string; access_token: string; refresh_token: string; account_id: string } }

  assert.equal(parsed.tokens.id_token, FAKE_ID_TOKEN)
  assert.equal(parsed.tokens.account_id, 'account-abc-123')
  // Never a real bearer shape — the access/refresh bearer stays a fixed, never-functional
  // placeholder; only the identity claims come from the stub.
...
test('ensureCodexAuthStub is idempotent — writing twice never fails', async () => {
  ...
  const stubsDir = await seedAuthStub(FAKE_ID_TOKEN, 'account-abc-123')
  await ensureCodexAuthStub(home, stubsDir)
  await assert.doesNotReject(() => ensureCodexAuthStub(home, stubsDir))
})

test('required: fails closed if the stub directory exists but the codex-auth-json stub file does not', async () => {
  const home = await mkdtemp(join(tmpdir(), 'codex-auth-stub-test-'))
  const emptyStubsDir = await mkdtemp(join(tmpdir(), 'onecli-stubs-empty-'))
  await assert.rejects(() => ensureCodexAuthStub(home, emptyStubsDir))
})

test('required: fails closed if the stub file is missing idToken or accountId', async () => {
  const home = await mkdtemp(join(tmpdir(), 'codex-auth-stub-test-'))
  const stubsDir = await mkdtemp(join(tmpdir(), 'onecli-stubs-'))
  await writeFile(join(stubsDir, 'codex-auth-json'), JSON.stringify({ idToken: FAKE_ID_TOKEN }))
  await assert.rejects(() => ensureCodexAuthStub(home, stubsDir))
})
