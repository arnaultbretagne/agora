...
/**
 * Test-only stand-in for `claude-agent-acp` as a real, separate, stdio-speaking process — proves
 * `bridge-server.ts`'s WS<->stdio plumbing, custody-triggering session-id tap and restore-before-
 * ready contract without needing live Claude Max credentials. `@agora/acp`'s fake Agent is OUR OWN
