...
Both fields and the source inventories of `observation.power` are defined in
[`000_taxonomy.md`](000_taxonomy.md). No other Intent field, Observation or historical fact may
change the result of `POWER`.
...
| `POWER-002` | `intent.power = off ∧ observation.power = on` | `ACTION(TURN_OFF)` |
| `POWER-003` | `intent.power = on ∧ observation.power = off` | `ACTION(TURN_ON)` |
...
`TURN_ON` and `TURN_OFF` are action names selected by this table. Their operational contracts are
specified separately; `POWER` does not reproduce the decisions of later rules inside either
action.
